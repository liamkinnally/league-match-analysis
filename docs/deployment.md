...
Before publication, build and run an isolated snapshot containing only the files staged for the release repository, using its own configuration and database. Reuse still-valid full-suite evidence when source is unchanged; rerun checks affected by corrections. Provider account creation, managed database provisioning, DNS, public ingress and publication remain pending.
...
Keep one persistent backend instance, PostgreSQL on a durable volume with scheduled backups, and backend-to-database traffic on Railway's private network. Keep service sleeping disabled. The existing Vercel domain/project used for other work must remain untouched; use a separate project and provider HTTPS subdomain initially.
...
- Attach a small volume to the backend at `/data` and retain one replica. Railway [prevents overlapping deployments for services with volumes](https://docs.railway.com/volumes/reference). This deliberately accepts brief redeploy downtime and preserves the existing startup recovery of interrupted lookups. The backend does not need to write to that mount; keep its non-root image user. **Do not remove this volume or enable replicas without replacing the singleton worker/recovery design.** Database persistence uses a separate PostgreSQL volume.
- PostgreSQL: use version 17, a private service hostname, durable volume and daily backups; avoid public database ingress. Keep backend and database in the same region and frontend functions nearby. Vercel config currently selects `iad1`.
