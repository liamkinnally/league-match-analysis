...
| Match-V5 `/lol/match/v5/matches/by-puuid/{puuid}/ids` | Up to twenty raw match IDs per page, optional queue filter, AMERICAS / EUROPE / ASIA routing |
...
| League-V4 `/lol/league/v4/entries/by-puuid/{puuid}` | Current queue-specific ranks on the selected platform |
...
2. Select NA, EUW, EUNE or KR and search for a Riot ID. Cached suggestions show up to five previously observed profiles; a profile URL can be shared directly. Loading may expose completed matches while remaining matches are retrieved. Open a result.
