...
export type PlayerProfile = { identity: ProfileIdentity; summoner: SummonerProfile; soloRank: SoloRank; flexRank: SoloRank; recentSolo: RecentSoloRecord; rankHistory: { trackingSince: string | null; observations: RankObservation[]; nextCursor: string | null } };
...
function parseRank(value: unknown): SoloRank {
  ...
  if (status === "ranked" && (values.tier === null || values.division === null || values.leaguePoints === null)) fail();
  if (row.winRate !== null && (typeof row.winRate !== "number" || !Number.isFinite(row.winRate) || row.winRate < 0 || row.winRate > 100)) fail();
  return { ...freshness(row), ...values, status, winRate: row.winRate as number | null, period: choice(row.period, ["unknown"] as const) };
}
const unavailableRank: SoloRank = { status: "unavailable", tier: null, division: null, leaguePoints: null, wins: null, losses: null, winRate: null, period: "unknown", fetchedAt: null, refreshing: false, stale: false, retryNotBefore: null, error: null };
export function parsePlayerProfile(value: unknown): PlayerProfile {
  const root = record(value), identity = record(root.identity), summoner = record(root.summoner), recent = record(root.recentSolo), history = record(root.rankHistory);
